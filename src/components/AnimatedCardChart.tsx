...
const Layer2: React.FC<{ color: string }> = ({ color }) => {
  return (
    <div className="group relative h-full w-[356px]" style={{ "--color": color } as React.CSSProperties}>
      <div className="ease-[cubic-bezier(0.6, 0, 1)] absolute inset-0 z-[7] flex w-[356px] translate-y-full items-start justify-center bg-transparent p-4 transition-transform duration-500 group-hover/animated-card:translate-y-0">
      ...
      <svg width="356" height="180" xmlns="http://www.w3.org/2000/svg">
        {rectsData.map((rect, index) => (
          <rect
            key={index}
            width={rect.width}
            height={hovered ? (rect as any).hoverHeight : rect.height}
            x={rect.x}
            y={hovered ? (rect as any).hoverY : rect.y}
            fill={hovered ? (rect as any).hoverFill : rect.fill}
            rx="2"
            ry="2"
...
// Convenience composition used by pages
...
function AnimatedCardChart({
  ...
  return (
    <AnimatedCard className={cn("w-full", className)}>
      <CardVisual className="h-[180px] w-full flex items-center justify-center">
        <Visual3 mainColor={mainColor} secondaryColor={secondaryColor} gridColor={gridColor} />
      </CardVisual>
      <CardBody>
