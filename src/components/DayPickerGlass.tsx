...
function Calendar({
  ...
  const mergedClassNames: typeof defaultClassNames = Object.keys(defaultClassNames).reduce(
    (acc, key) => ({
      ...
      [key]: (classNames as any)?.[key]
        ? cn((defaultClassNames as any)[key], (classNames as any)[key])
  ...
  const mergedComponents = { ...defaultComponents, ...(userComponents as any) };
...
export function MiniCalendarGlass(props: CalendarProps & { className?: string }) {
  return (
    <div
      className={cn(
        "w-full rounded-3xl p-5 bg-black/20 backdrop-blur-xl border border-white/10 text-white",
        "max-w-[420px]",
        props.className
      )}
    >
...
export { Calendar as DayPickerCalendar };
export default MiniCalendarGlass;
